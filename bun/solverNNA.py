...
def get_previous_teta2():
    ...
    # Remove empty string at the end if it exists
    if prev_teta and prev_teta[-1] == "":
        prev_teta.pop()

    # Convert string to float first, then to int to handle cases like '45.0'
    prev_teta = [int(float(i)) for i in prev_teta if i.strip()]
    return prev_teta
